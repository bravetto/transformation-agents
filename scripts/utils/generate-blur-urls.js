...
async function generateBlurDataURL(imagePath) {
  try {
    ...
    console.error(`Error generating blur data URL for ${imagePath}:`, error.message);
    return null;
  }
}
...
async function updatePersonImagesFile() {
  ...
  for (const person of people) {
    ...
    const imagePath = path.join('public', 'images', 'people', 'display', `${person.id}.webp`);
    
    // If WebP doesn't exist, try JPG
    if (!fs.existsSync(imagePath)) {
      const jpgPath = path.join('public', 'images', 'people', 'display', `${person.id}.jpg`);
  ...
  for (const personId in blurDataURLs) {
    ...
    const regex = new RegExp(`('${personId}':\\s*{[^}]*blurDataURL:\\s*)([^,}]*)`);
