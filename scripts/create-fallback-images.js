...
function copyFallbackImages() {
  ...
  for (const person of people) {
    for (const size of sizes) {
      const sourcePath = path.join(
        ...
        "images",
        "fallbacks",
        `${person.role}-fallback.jpg`,
      );

      // WebP target path
      const webpTarget = path.join(
        "public",
        "images",
        "people",
        size,
      ...
      try {
        if (process.platform === "darwin" || process.platform === "linux") {
          ...
          try {
            execSync(`cwebp -q 80 "${sourcePath}" -o "${webpTarget}"`, {
              stdio: "ignore",
            });
            webpCount++;
            ...
            console.log(
              `WebP conversion failed for ${webpTarget}, copying JPG instead`,
            );
  ...
  console.log(
    "These are temporary placeholders. Replace with actual images when available.",
  );
}
