...
import { useSpring, animated, useTrail, useChain, useSpringRef, config } from '@react-spring/web'
...
export function SacredReveal({ 
  ...
  const getInitialTransform = () => {
    switch (direction) {
      case 'up': return { transform: 'translateY(50px)', opacity: 0 }
      case 'down': return { transform: 'translateY(-50px)', opacity: 0 }
      case 'left': return { transform: 'translateX(50px)', opacity: 0 }
      case 'right': return { transform: 'translateX(-50px)', opacity: 0 }
      case 'scale': return { transform: 'scale(0.8)', opacity: 0 }
      default: return { transform: 'translateY(50px)', opacity: 0 }
    }
  }
...
export function PulsingHeart({ 
  ...
  const pulseConfig = intensity === 'gentle' 
  ...
  const springs = useSpring({
    ...
    config: { ...config.wobbly, duration: isClicked ? 300 : pulseConfig.duration },
...
export function ChainedAnimation({ 
  ...
  const { transform: transform2 } = useSpring({
    ...
    to: trigger ? { transform: 'translateY(0px)' } : { transform: 'translateY(50px)' },
    config: config.wobbly,
  })
  ...
  useChain(trigger ? [springRef1, springRef2, springRef3] : [springRef3, springRef2, springRef1], [0, 0.3, 0.6])
