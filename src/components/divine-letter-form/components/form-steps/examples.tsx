...
function Examples() {
  ...
  const guidance = useMemo(
    () => getGuidanceForRelationship(relationshipType),
    [relationshipType],
  );
  ...
  const hasDatePattern = (text: string): boolean => {
    const datePattern =
      /\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},\s+\d{4}\b|\b\d{1,2}\/\d{1,2}\/\d{2,4}\b/i;
    ...
    >
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600 mt-2">
          Provide specific examples that demonstrate JAHmere's character and
          impact
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        {errors.specificExample1 ? (
          ...
          <p className="text-xs text-gray-500 mt-1">
            Include a specific date (e.g., January 15, 2023) and concrete
            details
          </p>
        )}
      </div>
      ...
      <div className="bg-yellow-50 p-4 rounded-md mt-6">
        <p className="text-sm text-yellow-800">
          ...
          carry significantly more weight with the court than general
          statements.
        </p>
      </div>
    </motion.div>
